...
#define LED BIT6
#define SPEAKER_PIN BIT5  // Added for speaker
...
switch_interrupt_handler() {
    ...
    if(switches) {
        P1OUT ^= SPEAKER_PIN;  // Toggle P1.5 for sound
    } else {
        P1OUT &= ~SPEAKER_PIN;  // Turn off P1.5
    }
}
...
void main() {
    P1DIR |= LED | SPEAKER_PIN;  // Set P1.5 as output for speaker
    P1OUT |= LED;
